...
public class SearchFlight implements Task {
	...
	public void choosesNumberPassanger(Actor actor) {
		...
		try {
			...
			throw new PersonalizedException(
					"El numero de pasajeros solicitado  no está presente " + dataTest.get("numPasajeros").toString(),
					null);
		}
	...
	public void choosesDeparting(Actor actor) {
		...
		try {
			actor.attemptsTo(
					SelectFromOptions.byValue(dataTest.get("salida").toString()).from(FlightPage.SELECT_DEPARTING));
	...
	public void choosesDepartingOn(Actor actor) {
		...
		try {
			...
			actor.attemptsTo(
					SelectFromOptions.byValue(dataTest.get("diaSalida").toString()).from(FlightPage.SELECT_FROM_DAY));

		} catch (Exception e) {
			throw new PersonalizedException("la fecha de salida solicitado no se pudo seleccionar "
					+ dataTest.get("mesSalida").toString() + dataTest.get("diaSalida").toString(), e);
		}
	...
	public void choosesArraving(Actor actor) {
		...
		try {
			actor.attemptsTo(
					SelectFromOptions.byValue(dataTest.get("llegada").toString()).from(FlightPage.SELECT_ARRIVING));
	...
	public void choosesArravingOn(Actor actor) {
		...
		try {
			...
			actor.attemptsTo(
					SelectFromOptions.byValue(dataTest.get("diaLlegada").toString()).from(FlightPage.SELECT_TO_DAY));
